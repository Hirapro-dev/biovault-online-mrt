...
export default function ArchiveRegisterPage() {
  ...
  // 氏名のIME変換から読み仮名（ひらがな）を抽出するためのref
  const composeKana = useRef(""); // 変換中に捕捉した最新のひらがな読み
  const [phone, setPhone] = useState("");
  const [email, setEmail] = useState("");
  // 住所フィールド（郵便番号・住所1・住所2）
  ...
  // 視聴ID（ふりがなの苗字 + 電話下4桁から自動生成）
  const [viewerId, setViewerId] = useState("");

  // 視聴IDを手入力したか（手入力後は自動生成で上書きしない）
  const viewerIdEdited = useRef(false);

  // ふりがな・電話が揃ったら視聴IDを自動生成（手入力されていない場合のみ）
  useEffect(() => {
  ...
  // ① 氏名入力でふりがな（ひらがな）を自動反映
  // 漢字変換の確定文字ではなく、変換途中のひらがな読みを捕捉して反映する

  // ひらがな（+ 長音・スペース）のみで構成されるか判定
  const isHiraganaOnly = (s: string) => /^[ぁ-ゖー　\s]*$/.test(s);

  // 変換開始：読みバッファをリセットし、姓名のスペース区切りをふりがなにも反映
  ...
  // 捕捉したひらがな読みをふりがなに確定（refをクリアする前に値をキャプチャ）
  const commitKana = () => {
  ...
  // 変換確定：ひらがな・カタカナのまま確定したケースのフォールバック
  const handleNameCompositionEnd = () => {
    commitKana();
  };

  // 氏名入力：input イベントの data から読み仮名を抽出（最も確実な経路）
  const handleNameChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (native?.isComposing && data) {
      if (/[一-龯々]/.test(data)) {
        // 漢字に変換された → 直前のひらがな読みを確定してふりがなに追記
        commitKana();
      } else if (isHiraganaOnly(data)) {
        // 変換前のひらがな読みを保持（ローマ字途中・カタカナは無視）
        composeKana.current = data;
      }
    }
  };

  // ② 電話番号：数字のみ受け付け（ハイフン除去）
  const handlePhoneChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const digits = e.target.value.replace(/[^0-9]/g, "");
    setPhone(digits);
  };

  // ③ 郵便番号から住所自動反映（zipcloud API を使用）
